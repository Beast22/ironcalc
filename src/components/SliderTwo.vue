<template>
    <div class="slider-cont">
        <div 
            class="slider__but slider__minus"
            @click="decrementValue"
            >
            -
        </div>   
        <div 
            class="slider__but slider__plus"
...
let rationListArr = ref([])

export default {
    name: 'Slider',
    emits: ['update:modelValue', 'change', 'slideend', 'calc'],
    props: {
        modelValue: [Number, Array, Function],
        min: {
            type: Number,
            default: 0
        },
    ...
    methods: {
        ...
        setProduct(event){           
            const el = this.$el
            const sHandle = el.querySelector('.p-slider-handle')
            const sHandleNumbVal = Number(sHandle.getAttribute("aria-valuenow"))
            const prTitle = el.closest(".product").querySelector('.product-title').textContent 
            const sValue =  el.querySelector('.p-slider-handle').textContent  
            const sValNumb = sValue.split(/(\d+)/)   
            const arrVal = sValue.split(" ");  
            const sValNumd = Number(arrVal[0]) 
            const sValStr = arrVal[1]
            
            if(sHandleNumbVal > 0){
                let prodItem = document.createElement('li'); 
                    prodItem.setAttribute('class','prod-list');
                    prodItem.innerHTML = `
                    <span class="row-iron__result-text">${prTitle}</span> ................. <span class="row-iron__result-value">${sValue}</span> 
                    `;

                    let rationList = document.querySelector('.ration-list')

                    if (rationList.children.length == 0){
                        rationList.appendChild(prodItem);
                        rationListArr.value.push({prodtitle: prTitle, prodVal: sValue})
                        rationList.appendChild(prodItem);
                        console.log(rationListArr)
                    } else{
                        for (let i = 0; i < rationListArr.value.length; i++){
                            let hasProdTitle = rationListArr.value.some( item => item['prodtitle'] === prTitle )
                            const repeatIndex = rationListArr.value.findIndex(item => item['prodtitle'] === prTitle)
                            let prodItemLi = document.querySelector('.prod-list')

                            if (hasProdTitle){
                                rationListArr.value[repeatIndex].prodVal === this.modelValue + sValStr
                                const indexLi = Array.from(prodItemLi.parentNode.children).indexOf(prodItem)
                                let repeatedLi = rationList.getElementsByTagName('li')[repeatIndex]
                                repeatedLi.querySelector('.row-iron__result-value').textContent = sValNumd + sValStr
                                return false
                            } else{
                                console.log('go')
                                rationListArr.value.push({prodtitle: prTitle, prodVal: sValue})
    ...
    computed: {
        ...
        calcFe(){
            this.$emit('calcFe', rationListArr);
            // const sum = rationListArr.reduce((a, b) => a + b, 0);
             return 1111
        }
    }
};
